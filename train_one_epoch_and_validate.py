...
class ClipCocoDataset(Dataset):
    """
    Expects PKL with:
      - "clip_embedding": [N_images, D] tensor/ndarray
      - "captions": list of dicts {"image_id", "caption", "clip_embedding"} where CLIP index is in "clip_embedding"
    """
    def __init__(self, data_path: str, prefix_length: int, gpt2_type: str = "gpt2",
        ...
        self.image_ids = [c["image_id"] for c in captions_raw]
        self.captions = [c["caption"] for c in captions_raw]

        token_cache = f"{data_path[:-4]}_tokens.pkl"
        if os.path.isfile(token_cache):
            ...
            for c in captions_raw:
                toks = self.tokenizer.encode(c['caption'])
...
# Train one epoch
# ----------------------------
# --- change train_one_epoch signature to reuse optimizer/scheduler ---
...
# --- in main(), build optimizer/scheduler once and loop for epochs ---
def main():
    args = parse_args()
    save_config(args)

    # Datasets
    train_ds = ClipCocoDataset(args.train_pkl, args.prefix_length, normalize_prefix=args.normalize_prefix)
    val_ds   = ClipCocoDataset(args.val_pkl,   args.prefix_length, normalize_prefix=args.normalize_prefix)
